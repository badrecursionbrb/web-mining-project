...
X_train = vectorizer.fit_transform(train_data['tweet'], max_features=5000, max_df=0.8)
...
#%%
parameters = {
    'C': [0.1],# [0.001, 0.01, 0.1, 1, 10, 100],  # Regularization parameter
    'kernel': ['linear']  # Type of kernels
}
model = SVC()
grid_clf = GridSearchCV(model, parameters, cv=2, verbose=3, scoring='f1_weighted', n_jobs=-1)
...
grid_search_result = meta_grid_search(model=model, vectorizer_dict=vectorizer_dict, parameters=parameters, 
                            train_data=train_data, val_data=val_data, test_data=test_data, cv=2)
# %%
